import codeSrc from "../assets/codeimage.png"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const codeImage = document.querySelector("#code")

    clipboard.on('success', (e) => {
        document.querySelector(".contact-button").classList.add("copied-button")

        e.clearSelection();
    });

    const waveContainer =  document.querySelector(".waveCont")

    const section2Top = waveContainer.offsetTop + waveContainer.offsetHeight - 10
    document.querySelector(".section2").style.top = section2Top
    ...
    image.addEventListener('load', () => {
        console.log("EEEE")
        image.remove()
        codeImage.style.backgroundImage = `transparent url('${src}') 0% 0% no-repeat padding-box;`
        codeImage.classList.add("vis");
    });
    image.src = src;

    // rellax = new Rellax('#code');
});
